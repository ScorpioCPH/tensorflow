...
/* static */ StatusOr<se::Platform*> PlatformUtil::GetDefaultPlatform() {
  ...
  CPH_VLOG(1) << "GetDefaultPlatform.Size: " << platforms.size();
  if (platforms.empty()) {
    ...
    // the default.
    CPH_VLOG(1) << "platforms[0]->id(): " << platforms[0]->id();
    CPH_VLOG(1) << "platforms[1]->id(): " << platforms[1]->id();
    CPH_VLOG(1) << "se::host::kHostPlatformId: " << se::host::kHostPlatformId;
